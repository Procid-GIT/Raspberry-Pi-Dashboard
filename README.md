...
In my setup, I have a buildhat, attached to some lego spike prime motors, so it would grab the bricks it would see to sort them.  I hadn't coded those yet, so I will have to wait to see.  
For the Computer Vision, it works by capturing the image, as an array, to convert to a 128x128 image for the model to see.  The model then has layers at that size, for processing, using a 3, 3 shape and Conv2D.  It then flattens it, and passed
